...
- в обоих случаях пишется в копию репозиторя в Colab, скачаного ранее командой `!git clone https://github.com/Mike030668/Project_glass.git -q`
  После закрытия сессии, эти данный удвлятся
...
- Теперь можно проводить эксперементы. Если нужно скачать датаврейм тестовый с сигналами сеток, то можно просто сохранить датафрейм теста, указав нужный путь до папки и имя файла
  
 <img src="images/save_df.png" alt="png"  width="450"/>


## Общие рекомендации:
- нужно обратить внимание на более детальную подготовку данных для сеток. Вероятно нужно делать скейлин не 2мя скейлерами (не_стакан и стакан), а по каждому или группами колонкам (однородным)
- постоянною доучивание окном назад позволит сетке не прокисать.
- Нужно углубиться в оптимизацию разметки для тренда, пока тренд работает плохо, но эта задача намного сложнее, чем предсказание цены, которую мы постоянно подсказываем.
- стоит расмотреть другие варианты приземления предсказаний ценовой сетки.
